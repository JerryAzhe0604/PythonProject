...
from train_car import XMLDataset, create_ssd512  # Ensure train_car.py is present
...
def evaluate_car(model_path, test_folder, label_map, conf_threshold=0.4):
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    model, classes = load_eval_model(model_path, device)
    if not model: return

    print(f"✅ Loaded {model_path} ({classes} classes). Starting Car Evaluation...")
    ...
    for images, targets in tqdm(loader):
        ...
        for out in outputs:
            ...
            res.append({"boxes": out["boxes"][keep].cpu(), "scores": out["scores"][keep].cpu(),
    ...
    print(
        f"\nCAR RESULTS: Precision: {results['map'].item():.4f} | Recall: {results['mar_100'].item():.4f} | mAP@50: {results['map_50'].item():.4f}")


if __name__ == "__main__":
    # Car IDs are usually 2 in your setup
    CAR_MAP = {'cars': 2, 'car': 2, 'vehicle': 2}
    evaluate_car("car_specialist.pth", r"dataset/Car Models.v2-carobject.voc/valid", CAR_MAP)