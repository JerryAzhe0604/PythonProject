...
def evaluate_plate(model_path, test_folder, label_map, conf_threshold=0.3):
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    model, classes = load_eval_model(model_path, device)
    if not model: return

    print(f"✅ Loaded {model_path} ({classes} classes). Starting Plate Evaluation...")
    ...
    print(
        f"\nPLATE RESULTS: Precision: {results['map'].item():.4f} | Recall: {results['mar_100'].item():.4f} | mAP@50: {results['map_50'].item():.4f}")


if __name__ == "__main__":
    # Plate IDs are usually 1
    PLATE_MAP = {'plate': 1, 'license-plate': 1}
    evaluate_plate("ssd512_plate.pth", r"dataset/License-Plate.v1i.voc/valid", PLATE_MAP)