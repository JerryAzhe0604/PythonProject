...
st.write("Synced with RESCUE Architecture. Detecting Color, Brand, and Plate.")
...
# --- THE SYNCED ARCHITECTURE (Matches RESCUE Weights) ---
def create_model(num_classes):
    backbone_vgg = vgg16(weights=None).features

    # Matches "backbone.features" naming in your .pth file
    ...
    # [4, 6, 6, 6, 4, 4, 4] fixes the 44 vs 66 parameter error
...
def load():
    ...
    if os.path.exists(MODEL_PATH):
        # strict=False allows loading despite backbone naming differences
...
threshold = st.sidebar.slider("Confidence Threshold", 0.0, 1.0, 0.25)
...
if file:
    ...
    for box, label, score in zip(output['boxes'], output['labels'], output['scores']):
        if score > threshold:
            ...
            d = {'box': b.tolist(), 'label': label.item(), 'score': score.item()}
            if d['label'] == 1:
                plates.append(d)
            elif d['label'] == 2:
                cars.append(d)
    ...
    for car in cars:
        ...
        display_name = f"{color} {brand_text} CAR".strip().replace("  ", " ")
        ...
        detected_list.append({"Type": "Vehicle", "Conf": f"{car['score']:.1%}", "Info": display_name})
    ...
    for p in plates:
        ...
        detected_list.append({"Type": "Plate", "Conf": f"{p['score']:.1%}", "Info": plate_no})

    # Output
    c1, c2 = st.columns([3, 1])
    c1.image(img_pil, use_container_width=True)
    if detected_list:
        c2.dataframe(detected_list, use_container_width=True)
    else:
        max_s = output['scores'][0].item() if len(output['scores']) > 0 else 0
        st.sidebar.warning(f"No detections. Highest score: {max_s:.2%}")